...
local config = {
    ...
    autoexport = false,            -- enable autoexport to anki when mpv starts
...
autoexport = (function()
    local enable = function()
        mp.observe_property("sub-text", "string", my_export_to_anki)
    end

    local disable = function()
        mp.unobserve_property(my_export_to_anki)
    end

    local state_notify = function()
        notify(string.format("Autoexport has been %s.", config.autoexport and 'enabled' or 'disabled'))
    end

    local toggle = function()
        config.autoexport = not config.autoexport
        if config.autoexport == true then
            enable()
        else
            disable()
        end
    ...
    local is_enabled = function()
        return config.autoexport == true and 'enabled' or 'disabled'
    end
...
menu.keybindings = {
    ...
    { key = 'u', fn = function() autoexport.toggle() end },
...
menu.update = function()
    ...
    osd:item('Autoexport: '):text(autoexport.is_enabled()):newline()
    ...
    elseif menu.hints_state.get() == 'menu' then
        ...
        osd:tab():item('u: '):text('Toggle autoexport'):newline()
...
local main = (function()
    ...
    return function()
        ...
        mp.add_key_binding(nil, "mpvacious-toggle-autoexport", autoexport.toggle)
