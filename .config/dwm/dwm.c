...
applysizehints(Client* c, int* x, int* y, int* w, int* h, int interact)
{
  ...
  *h = MAX(1, *h);
  ...
  if (*w < bh)
    *w = bh;
  if (resizehints || (c->isfloating && !(c->tags & SPTAGMASK)) ||
...
arrangefixedfloat(Monitor* m)
{
  ...
  for (; c; c = c->next)
    if (c->tags & SPTAGMASK && ISVISIBLE(c) && !c->isfullscreen &&
	...
	b += (iv + 1) / 2;
      resize(c,
             m->wx + 2 * oh + l,
             m->wy + 2 * ov + t,
             m->ww - 4 * oh - 2 * c->bw - l - r,
             m->wh - 4 * ov - 2 * c->bw - t - b,
             0);
    }
}
...
drawbar(Monitor* m)
{
  ...
  for (i = 0; i < LENGTH(scratchpads); i++) {
    ...
    drw_setscheme(
      drw,
      scheme[selmon->tagset[selmon->seltags] & scratchtag ? SchemeSel
                                                          : SchemeNorm]);
    drw_text(
      drw, x, 0, w, bh, lrpad / 2, scratchpads[i].name, urg & scratchtag);
...
incnmaster(const Arg* arg)
{
  selmon->nmaster = selmon->pertag->nmasters[selmon->pertag->curtag] =
    MIN(MAX(selmon->nmaster + arg->i, 1), 5);
  arrange(selmon);
}
